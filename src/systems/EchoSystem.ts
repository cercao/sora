...
import { BIOMES } from '../world/WorldConfig';
...
export class EchoSystem {
  ...
  constructor(scene: Phaser.Scene, spirit: Spirit, sound: AmbientSound) {
    this.scene = scene;
    this.spirit = spirit;
    this.sound = sound;
    this.createHUD();
  ...
  private createHUD(): void {
    ...
    bg.fillRoundedRect(-240, 0, 230, 28 + BIOMES.length * 30, 8);
  ...
  private createMarker(echo: EchoData): void {
    const biome = BIOMES.find(b => b.id === echo.island);
